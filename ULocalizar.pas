    ...
    for i:=0 to Self.ComponentCount-1 do
      begin
        ...
        if (((objNome = 'cbCampo') and (objConteudo = EmptyStr))
          ...
          begin
            Application.MessageBox(PChar('O campo '''+objHint+''' não pode ser vazio'), 'Aviso', MB_ICONEXCLAMATION + MB_OK);
            if (Self.Components[i] is TComboBox) then
              begin
        ...
        if AddFiltro = True then
          begin
            ...
            if Criterio = 'é igual' then
              Filtro := Filtro+' = '''+Valor1+''''
            else if Criterio = 'não é igual' then
              Filtro := Filtro+' <> '''+Valor1+''''
            else if Criterio = 'contém' then
              Filtro := Filtro+' LIKE '''+Valor1+''''
            else if Criterio = 'não contém' then
              Filtro := Filtro+' NOT LIKE '''+Valor1+''''
            else if Criterio = 'está entre' then
              Filtro := Filtro+' BETWEEN '''+Valor1+''' AND '''+Valor2+''''
            else if Criterio = 'não está entre' then
              Filtro := Filtro+' NOT BETWEEN '''+Valor1+''' AND '''+Valor2+''''
            else if Criterio = 'é nulo' then
               Filtro := Filtro+' IS NULL'
            else if Criterio = 'não é nulo' then
              Filtro := Filtro+' IS NOT NULL'
            else if Criterio = 'é verdadeiro' then
              Filtro := Filtro+ ' = true'
            else if Criterio = 'é falso' then
              Filtro := Filtro+ ' = false';
